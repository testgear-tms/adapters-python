...
from testgear_adapter_pytest import LocalPluginManager


def inner(function):
    @wraps(function)
    def wrapper(*args, **kwargs):
        if hasattr(LocalPluginManager.get_plugin_manager().hook, 'add_parameters') and kwargs:
            LocalPluginManager.get_plugin_manager().hook.add_parameters(parameters=kwargs)
...
def addLink(url: str, title: str = None, type: str = None, description: str = None):
    if hasattr(LocalPluginManager.get_plugin_manager().hook, 'add_link'):
        LocalPluginManager.get_plugin_manager().hook.add_link(link_url=url, link_title=title, link_type=type, link_description=description)


def message(test_message: str):
    if hasattr(LocalPluginManager.get_plugin_manager().hook, 'add_message'):
        LocalPluginManager.get_plugin_manager().hook.add_message(test_message=test_message)
...
def attachments(*attachments_paths: str):
    if step.step_is_active():
        ...
        if hasattr(LocalPluginManager.get_plugin_manager().hook, 'add_attachments'):
            LocalPluginManager.get_plugin_manager().hook.add_attachments(attach_paths=attachments_paths)
...
class step:
    ...
    def __exit__(self, exc_type, exc_value, tb):
        outcome = 'Failed' if exc_type else LocalPluginManager.get_plugin_manager().hook.get_pytest_check_outcome()[0] if\
            hasattr(LocalPluginManager.get_plugin_manager().hook, 'get_pytest_check_outcome') else 'Passed'
    ...
    def add_attachments(cls, attachments_paths):
        if hasattr(LocalPluginManager.get_plugin_manager().hook, 'load_attachments'):
            cls.attachments[-1] += LocalPluginManager.get_plugin_manager().hook.load_attachments(attach_paths=attachments_paths)[0]
