...
from testgear_adapter_pytest.utils import (
    autotests_parser,
    search_in_environ,
    step,
    url_check,
...
class PluginListener(object):
    ...
    def __init__(self, testrun, url, private_token, configuration, proxy=None):
        if testrun:
            ...
            url = url_check(search_in_environ(parser.get('testgear',
                                                         'url')))

            private_token = search_in_environ(parser.get('testgear',
                                                         'privatetoken'))
            try:
            ...
            if parser.has_option('testgear', 'testrunID'):
                self.testrun_id = uuid_check(search_in_environ(
                    parser.get('testgear', 'testrunID')))
            else:
                self.project_id = uuid_check(
                    search_in_environ(parser.get('testgear', 'projectID')))
                self.configuration_id = uuid_check(
                    search_in_environ(parser.get('testgear', 'configurationID')))

                if parser.has_option('testgear', 'testrun_name'):
                    self.testrun_name = parser.get('testgear', 'testrun_name')
    ...
    def pytest_runtest_protocol(self, item):
        if hasattr(item.function, 'test_external_id'):
            if not hasattr(item.function,
                ...
                self.item = None
                raise Exception(
                    f'{item.originalname} must have @testgear.displayName or documentation!')
            self.item = item
        else:
            self.item = None
    ...
    def _set_links(item, data):
        if hasattr(item, 'array_parametrize_mark_id'):
            for link in item.function.test_links:
                data['links'].append({})
                data['links'][-1]['url'] = PluginListener.attribute_collector_links(
                                                link,
                                                'url',
                                                item.own_markers,
                                                item.array_parametrize_mark_id,
                                                item.index)
                if link['title']:
                    data['links'][-1]['title'] = PluginListener.attribute_collector_links(
                                                    link,
                                                    'title',
                                                    item.own_markers,
                                                    item.array_parametrize_mark_id,
                                                    item.index)
                if link['type']:
                    data['links'][-1]['type'] = PluginListener.attribute_collector_links(
                                                    link,
                                                    'type',
                                                    item.own_markers,
                                                    item.array_parametrize_mark_id,
                ...
                if link['description']:
                    data['links'][-1][
                        'description'] = PluginListener.attribute_collector_links(
                                            link,
                                            'description',
                                            item.own_markers,
                                            item.array_parametrize_mark_id,
    ...
    def _get_title_from(item):
        if not hasattr(item.function, 'test_title'):
            return None
        if hasattr(item, 'array_parametrize_mark_id'):
            return PluginListener.param_attribute_collector(item.function.test_title,
                                                            item.own_markers,
                                                            item.array_parametrize_mark_id,
                                                            item.index)
    ...
    def _get_description_from(item):
        if not hasattr(item.function, 'test_description'):
            return None
        if hasattr(item, 'array_parametrize_mark_id'):
            return PluginListener.param_attribute_collector(
    ...
    def _set_labels(item, data):
        if hasattr(item, 'array_parametrize_mark_id'):
            result, param_id = PluginListener.mass_param_attribute_collector(
    ...
    def attribute_collector(item):
        ...
        data = {
            ...
            'steps': PluginListener._get_steps_from(item),
            'stepResults': PluginListener._get_step_results_from(item),
            'setUp': PluginListener._get_setup_from(item),
            'setUpResults': PluginListener._get_setup_results_from(item),
            'tearDown': PluginListener._get_teardown_from(item),
            'tearDownResults': PluginListener._get_teardown_results_from(
                item),
            'resultLinks': PluginListener._get_result_links_from(item),
            'duration': PluginListener._get_duration_from(item),
            ...
            'traces': PluginListener._get_traces_from(item),
            'namespace': item.function.__module__,
            'attachments': PluginListener._get_attachments_from(item),
            'parameters': PluginListener._get_parameters_from(item),
            'classname': PluginListener._get_classname_from(item),
            'title': PluginListener._get_title_from(item),
            'description': PluginListener._get_description_from(item),
            'links': [],
            'labels': [],
            'workItemsID': [],
            'message': None
        ...
        if hasattr(item.function, 'test_links'):
            PluginListener._set_links(item, data)

        if hasattr(item.function, 'test_labels'):
            PluginListener._set_labels(item, data)

        if hasattr(item.function, 'test_workitems_id'):
            PluginListener._set_workitems_id(item, data)
        ...
        if hasattr(item.function, 'test_displayname'):
            if hasattr(item, 'array_parametrize_mark_id'):
                data['autoTestName'] = PluginListener.param_attribute_collector(
            ...
            data['traces'] = '>\n{}\nE {} must have @testgear.displayName or documentation!\n{}:{}: Exception'.format(
    ...
    @testgear_adapter_pytest.hookimpl
